  ...
  <Card >
    <CardHeader>
      <div class="flex flex-row justify-between gap-2 items-center">
        ...
        <Tooltip v-if ="movie && movie.imdbID">
          <TooltipTrigger>
    ...
    >
    <CardContent>
      <Skeleton v-if="!movie || !movie.Title" class="w-full h-96"</Skeleton>
  ...
  methods: {
    async fetchMovie() {
      const response = await fetch(
        `http://www.omdbapi.com/?i=${this.imdbID}&apikey=${API_KEY}`
      );
      const data = await response.json();
      if (response.status !== 200 || data.Response === false) {
